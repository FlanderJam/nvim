...
later(function()
  ...
  vim.keymap.set("n", "<leader>sn", function()
    MiniPick.builtin.files({ source = { items = vim.fn.stdpath("config") } })
  end, { desc = "[S]earch [/] in Open Files" })
end)
...
later(function()
  require("config.plugins.formatter")(add)
end)
later(function()
  require("config.plugins.linter")(add)
end)

--- This function is to help properly set up deno formatters and linters if a
--- deno project is opened.
--- By default, it is assumed to be a node project.
later(function()
  local is_deno_project = vim.fs.root(0, { "deno.json", "deno.jsonc", "deno.lock" })
  if is_deno_project ~= nil then
    local conform = require("conform")
    conform.formatters_by_ft.javascript = { "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.javascriptreact = { "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.typescript = { "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.typescriptreact = { "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.markdown = { "vale", "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.json = { "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.jsonc = { "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.css = { "deno_fmt", lsp_format = "fallback" }
    conform.formatters_by_ft.html = { "deno_fmt", lsp_format = "fallback" }
